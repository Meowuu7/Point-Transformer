...
class Indoor3DSemSeg(data.Dataset):
    def __init__(self, num_points, train=True, download=True, data_precent=1.0):
        ...
        if not os.path.exists(os.path.join(self.data_dir, "all_data.npy")):
            ...
            np.save(os.path.join(self.data_dir, "all_data.npy"), data_batches)
            np.save(os.path.join(self.data_dir, "all_labels.npy"), labels_batches)
            print("Saved!")
        else:
            data_batches = np.load(os.path.join(self.data_dir, "all_data.npy"))
            labels_batches = np.load(os.path.join(self.data_dir, "all_labels.npy"))
            print("Loaded from saved npy files.")
    ...
    def __getitem__(self, idx):
        ...
        np.random.shuffle(pt_idxs) # random shuffle --- serves as one data augmentation method
