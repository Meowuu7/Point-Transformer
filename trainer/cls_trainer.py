...
class TrainerClassification(nn.Module):
    def __init__(self, dataset_root, num_points=1024, batch_size=32, num_epochs=200, cuda=None, dataparallel=False,
        ...
        self.model_dir = "init_lr_{}_bsz_{}_drop_lr_120_{}_160_{}_Adam_{}_50_lr_schedule_projection_aug_more_{}_nfeat_{}_weight_decay_{}_with_dropout_{}_more_bn_attn_mult_{}_use_abs_pos_{}_with_normal_{}_resume_{}".format(str(self.init_lr),
                                                                          str(batch_size),
    ...
    def _train_one_epoch(self, epoch):
        ...
        for batch_data, batch_label in train_bar:
            ...
            #     batch_label = torch.from_numpy(batch_label).long().to(self.device)
            # else:
            #     batch_x = torch.from_numpy(batch_data).float()
            #     batch_pos = torch.from_numpy(batch_data).long()
    ...
    def save_model(self, epoch):
        torch.save(self.model.state_dict(), os.path.join(self.model_dir, "checkpoint_epoch_{:d}.pth".format(epoch)))
